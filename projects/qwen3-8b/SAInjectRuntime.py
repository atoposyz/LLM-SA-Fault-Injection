    ...
    if random_mode or layertype == "all":
        for kernel_name, kernel_path in pos_mapping.items():
            try:
                targets.append(
                    (
                        f"layers.{layernumber}.{kernel_name}",
                        get_module_by_path(layer_module, kernel_path),
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--layerList", type=int, nargs="+", default=[0],
        help="输入错误注入层数，默认为0"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--affect", action="store_true",
        help="是否开启注入后续层数，默认为False"
    )
    parser.add_argument(
        "--layerType", type=str, choices=list(pos_mapping.keys()) + ["all"],
        help="注入层类型"
    )
    parser.add_argument(
        "--outputfile", type=str,
    ...
    parser.add_argument(
        "--pe", type=int, nargs=2, default=[-1, -1],
        help="手动指定注入PE位置"
    )
    parser.add_argument(
        "--reg", type=str, choices=["input", "weight", "psum"], default="weight"
    )
    parser.add_argument(
        "--dataflow", type=str, default="WS", choices=["WS", "OS", "IS"]
    )
    parser.add_argument(
        "--injectConfig", type=str, default="weight_bitflip_10"
    )
    parser.add_argument(
        "--random", action="store_true", help="开启随机注错模式"
    )
    parser.add_argument(
        "--ber", type=int, default=1, help="随机注错错误率"
    )
    parser.add_argument(
        "--fixReg", type=str,
        choices=["mixed", "input", "weight", "psum"], default="mixed"
    )
    parser.add_argument(
        "--fixDataflow", type=str,
        choices=["random", "WS", "OS", "IS"], default="random"
    )
    parser.add_argument(
        "--precision", type=str, default="bf16",
        choices=["fp32", "bf16"], help="注错精度，默认bf16"
    )
    parser.add_argument(
    ...
    if args.random:
        result_output_path = (
            f"{out_path}all_layer_random_reg{args.fixReg}_df{args.fixDataflow}.jsonl"
        )
    else:
        result_output_path = f"{out_path}all_layer_{layertype}.jsonl"
    ...
    writer = RuntimeMetricsWriter(
        ...
        run_tag="inject",
        extra_meta={"script": "SAInjectRuntime", "model": MODEL_ID},
        flush_every=1,
    )
    ...
    detect_metric_hook = build_metrics_hook(
        writer, matrix_collector, lambda: current_meta, injector
    )
    ...
    for idx, sample in enumerate(tqdm(samples)):
        ...
        if args.random:
            ...
            injector.dataflow = (
                ...
                else args.fixDataflow
            )

            if args.fixReg == "mixed":
                injector.init_multi_fault_positions(
                    num_faults=args.ber, num_regs=3, num_bits=32
                )
            else:
                reg_id = reg_map.get(args.fixReg, 1)
                injector.init_multi_fault_positions(
                    num_faults=args.ber, num_regs=1, num_bits=32
                )
        ...
        if args.affect and not args.random:
            target_layers = list(
                range(layerlist[0], model.config.num_hidden_layers)
            )

        for layernumber in target_layers:
            layer_module = model.model.layers[layernumber]
            for hook_name, hook_module in resolve_injection_targets(
                layernumber, current_layertype, layer_module, args.random
            ):
        ...
        for detect_layer in detect_layer_list:
            ...
            handles.append(
                detect_module.register_forward_hook(detect_metric_hook)
            )
        ...
        with open(result_output_path, "a", encoding="utf-8") as handle:
            ...
            result_data = {
                ...
                "fault_pe_reg": (
                    str(injector.fault_reg)
                    if len(injector.fault_reg) > 1
                    else (
                        injector.fault_reg[0] if injector.fault_reg else 0
                    )
                ),
                "dataflow": injector.dataflow,
