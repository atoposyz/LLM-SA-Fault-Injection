...
=============================================================================
  ...
  - sr_cfg 默认值: interval=50, threshold=1.2, cmp="<=", count_mode="consecutive",
    trigger_k=3, window_n=10, detect_layer=0
  - 重新生成阶段: do_sample=False (贪心解码)
...
=============================================================================
  ...
  --interval       StableRank计算间隔      (int, default=50)
  --detectLayer    计算StableRank的层ID    (int, default=0)
  --max_tokens     最大生成token数         (int, default=5000)
  --sr_threshold   StableRank阈值          (float, default=1.2)
  --sr_cmp         StableRank比较运算符    (choices: >=/<=/>/<, default="<=")
  --sr_count_mode  计数方式                (choices: consecutive/cumulative/window,
                                           default="consecutive")
  --sr_trigger_k   触发提前终止所需次数K    (int, default=3)
  --sr_window_n    window模式窗口大小N     (int, default=10, 仅window模式有效)

=============================================================================
程序行为：
=============================================================================
  1. 加载模型和数据集
  2. 在 detectLayer 上注册 OutputStableRankTest hook，每隔 interval 个 token 计算
     该层输出的 StableRank (= Frobenius范数² / 谱范数²)
  3. 在目标层 target_layers 上注册 fault injector hook
  4. 逐 token 生成（max_tokens 步），每个 step 只输入最后一个 token + KV cache
  5. 当 StableRank 异常次数达到 sr_trigger_k 次时，触发 StopForwardException
     → 先保存被中断的结果（is_aborted=True）
     → 再用 model.generate() 无注入重新生成一次（is_aborted=False, is_rerun=1）
...
=============================================================================
  ...
  uv run python projects/rerun/SARerun.py
  ...
  # 4. 调试：只跑指定样本0,5,10，max_tokens=500，用累计计数触发
  uv run python projects/rerun/SARerun.py \
      --sampleid 0 5 10 \
      --max_tokens 500 \
      --sr_count_mode cumulative \
      --sr_trigger_k 5 \
      --sr_threshold 1.5 \
      --sr_cmp "<="

  # 5. window模式：窗口10，触发5次，监测layer3输出
  uv run python projects/rerun/SARerun.py \
      --layerList 3 \
      --detectLayer 3 \
      --sr_count_mode window \
      --sr_window_n 10 \
      --sr_trigger_k 5 \
      --sr_threshold 1.0

  # 6. psum注入，注入ffn-down层，指定输出目录
  uv run python projects/rerun/SARerun.py \
      --layerList 1 \
      --layerType ffn-down \
      --reg psum \
...
def greedy_generate(model, tokenizer, inputs, max_new_tokens):
    ...
    with torch.no_grad():
        for step in range(max_new_tokens):
            if past_key_values is None:
                model_inputs = {"input_ids": generated, "use_cache": True}
            else:
                model_inputs = {
...
def resultoutput(token_length, is_aborted, decoded, idx, prompt, sample, layerid, layertype, posid, f, elapsed_time=0.0, stop_reason=None, is_rerun=0, repeated_token_id=None):
    sep = 'assistant\n'
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_suffix", type=str, default="guarded", help="输出文件名附加后缀；空字符串表示不附加")
    ...
    try:
        tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-8B", trust_remote_code=True, local_files_only=True)
        model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen3-8B", torch_dtype=torch.bfloat16, device_map="auto", trust_remote_code=True, local_files_only=True)
        print("[cache] 使用本地缓存加载 Qwen/Qwen3-8B")
    except (OSError, EnvironmentError, RuntimeError) as e:
        print(f"[cache] 本地缓存加载失败，尝试远程加载: {e}")
    ...
    random.seed(37)
    start_sample_id = 581
    all_samples = random.sample(list(ds), 1000)[0:1000]
    samples = all_samples[start_sample_id:1000]
    print("[INFO] 数据集已加载")
    ...
    if sample_index:
        ...
        try:
            samples = [all_samples[i] for i in sample_index]
        except IndexError as e:
            raise ValueError(f"指定的样本ID越界：{e}")
    else:
        print("[INFO] 未指定样本ID，使用随机选择的全部样本进行注错测试")
        sample_index = [str(i) for i in range(start_sample_id, 1000)]  # 生成样本索引列表
        ...
        with open(output_path, "a", encoding="utf-8") as f:
            for idx, sample in enumerate(tqdm(samples)):
                ...
                try:
                    ...
                    resultoutput(token_length=token_length, is_aborted=False, decoded=decoded, idx=sample_index[idx], prompt=prompt, sample=sample, layerid=layerid, layertype=layertype, posid=posid, f=f, elapsed_time=time.time() - t_start, stop_reason=stop_reason, is_rerun=0)
                    ...
                    token_length = generated.shape[1] - num_tokens
                    resultoutput(token_length=token_length, is_aborted=True, decoded=decoded, idx=sample_index[idx], prompt=prompt, sample=sample, layerid=layerid, layertype=layertype, posid=posid, f=f, elapsed_time=time.time() - t_start, stop_reason="stable_rank_abort", is_rerun=0)
                    # pass
                    # 重新运行，重置相关变量
                    sr_state["value"] = 0
                    sr_state["buf"] = None
                    ...
                    outputs, rerun_stop_reason = greedy_generate(
                        model=model,
                        tokenizer=tokenizer,
                        inputs=inputs,
                    ...
                    resultoutput(token_length=token_length, is_aborted=False, decoded=decoded, idx=sample_index[idx], prompt=prompt, sample=sample, layerid=layerid, layertype=layertype, posid=posid, f=f, elapsed_time=time.time() - t_rerun_start, stop_reason=rerun_stop_reason, is_rerun=1)
