...
pos_mapping = {
    ...
    "v": "self_attn.v_proj", 
    "o": "self_attn.o_proj",
    "qkv": "linear_attn.in_proj_qkv", # 用于 Linear Attention 层的合并投影
...
if __name__ == "__main__":
    ...
    injector = SingleBit_Fast_SA_FaultInjector(
        ...
        fault_type=args.injectConfig, precision='fp32'
    )
    ...
    layerid = args.layerList[0]
    posid = injector.fault_config.get('pos', 0)
    output_path = f"{out_path}single_bit_{layertype}.jsonl"
            ...
            for layernumber in target_layers:
                ...
                if current_layertype in ["q", "k", "v", "o", "qkv"]:
                    if layernumber % 4 == 3 and current_layertype == "qkv":
                        ...
                        current_layertype = "q"
                    elif current_layertype in ["q", "k", "v", "o"] and layernumber % 4 != 3:
                        # 第 0, 1, 2 层是 Linear Attention，使用 qkv 聚合
                ...
                if current_layertype in pos_mapping:
                    try:
                        ...
                        print(f"[Warning] Hook 挂载失败，层级 {layernumber}，路径 {pos_mapping[current_layertype]}。报错: {e}")
                        continue
