...
class SingleBit_Fast_SA_FaultInjector:
    """
    针对单比特（Single-bit）的脉动阵列故障注入器。
    优化了单点注入性能，修复了符号位溢出Bug，移除了不必要的张量开销。
    """
    ...
    def print_config(self):
        print(f"Single-Bit Fast SA Fault Injector:")
        print(f"  SA Size: {self.sa_rows}x{self.sa_cols} | Dataflow: {self.dataflow}")
        print(f"  Fault Type: {self.fault_type_str} | Enabled: {self.enabled}")
        if self.fault_pe_row:
            print(f"  Injected Position -> Row: {self.fault_pe_row[0]}, Col: {self.fault_pe_col[0]}, Bit: {self.fault_bit[0]}")
        else:
            print("  No fault position initialized yet.")

    def parse_fault_type(self):
        parts = self.fault_type_str.split('_')
        mode, f_type = parts[0], parts[1]
        op = 'flip'
        pos = 0
        if f_type == 'bitflip':
            op = 'flip'
