...
class FaultInjectionRuntimeError(RuntimeError):
    """Raised when the injector cannot safely finish a hook call."""
    pass
...
class Fast_SA_FaultInjector:
    ...
    def set_specific_fault(self, row: int, col: int):
        self.set_fault_position(row, col)
    ...
    def _inject_bit_error(self, tensor: torch.Tensor, mask: torch.Tensor, op: str):
        ...
        tensor_fp32 = torch.nan_to_num(tensor_fp32, nan=0.0, posinf=1e3, neginf=-1e3)
    ...
    def _signed_int32_mask(self, bit_pos: int) -> int:
        if not (0 <= bit_pos < 32):
            raise ValueError(f"bit position out of fp32 range: {bit_pos}")
        bit = 1 << bit_pos
        return bit - (1 << 32) if bit >= (1 << 31) else bit
    ...
    def _build_pe_mask_map(self, device):
        ...
        for f_idx in range(len(self.fault_pe_row)):
            ...
            bit = self._signed_int32_mask(mask_pos)
    ...
    def hook_fn(self, module, input_tuple, output_tuple):
        ...
        try:
            ...
            self.enabled = False
            if self.fail_closed:
                raise FaultInjectionRuntimeError(f"fault injection hook failed safely: {exc}") from exc
            return output_tuple
...
SingleBit_Fast_SA_FaultInjector = Fast_SA_FaultInjector
